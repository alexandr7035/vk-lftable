...
def start_text():
    # Send invitation
    text = '🗓 VK-LFTable v' + lftable_version + ': быстрый доступ к расписанию занятий юридического факультета БГУ.\n'
    text += "⌨️ Нажимте кнопку 'Start', чтобы начать работу.\n\n"

    text += '⚠ Управление ботом происходит посредством клавиатуры, однако на данный момент многие приложения, включая Kate Mobile и VK mp3, ее не поддерживают.\n'
